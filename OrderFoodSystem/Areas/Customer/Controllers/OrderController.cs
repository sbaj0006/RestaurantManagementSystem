...
namespace OrderFoodSystem.Areas.Customer.Controllers
{
    [Area("Customer")]
    public class OrderController : Controller
    {
        ...
        public object OrderDetailsVewModel { get; private set; }
        ...
        [Authorize]
        public async Task<IActionResult> OrderHistory(int productPge = 1)
        {
            var claimsIdentity = (ClaimsIdentity)User.Identity;
            var claim = claimsIdentity.FindFirst(ClaimTypes.NameIdentifier);
            ...
            orderListVM.Orders = orderListVM.Orders.OrderByDescending(p => p.OrderHeader.Id).Skip((productPge - 1) * PageSize).Take(PageSize).ToList();
            orderListVM.PagingInfo = new PagingInfo
            {
                CurrentPage = productPge,
